...
token = '6245482010:AAENa5JopZ_apCE_Mhn6TnLS49_RlaSvCJw' #bot token from @BotFather
...
async def welcome(message: types.Message):
    ...
    await message.sticker('static/hi.webp')

    await message.answer(
                     "Добро пожаловать, {0.first_name}!\nЯ - <b>{1.first_name}</b>, бот дурачок."
...
@dp.message_handler(func=lambda message: message.text == "Расскажи о погоде")
def weather(message):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            bot.send_message(message.chat.id,
                             text=f'Сейчас в городе {getLoc} {fact["temp"]}°, ощущается как {fact["feels_like"]}°. Сейчас на улице {fact["condition"]}. Скорость ветра: {fact["wind_speed"]}.'
                                  f'"Восход: {forecast["sunrise"]}. Закат: {forecast["sunset"]}')
        else:
...
@dp.message_handler(func=lambda message: message.text == "Игра Камень, ножницы, бумага")
...
@dp.message_handler(func=lambda message: message.text == "Отправить Настюше мем")
def mem(message):
    markup = types.InlineKeyboardMarkup()
...
@dp.message_handler(func=lambda message: message.text == "Поговорим?")
def talk(message):
...
@dp.message_handler(func=lambda message: message.text == "Как дела?")
def lala(message):
...
@dp.message_handler(func=lambda message: message.text == "Подскажи время")
def time(message):
    markup = types.InlineKeyboardMarkup(row_width=2)
...
@dp.message_handler(func=lambda message: message.text == "Игра Угадай число")
...
@dp.message_handler(func=lambda message: message.text == "Повторюшка")
def repeat(message):
